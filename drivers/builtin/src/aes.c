                           ...
                           unsigned int keybits)
{
    ...
    if(mbedtls_aeszkn_has_support()) {
        return mbedtls_aeszkn_setkey_enc((unsigned char *)RK, key, keybits);
    }
#endif
...
#if defined(MBEDTLS_AESZKN_HAVE_CODE)
    if (mbedtls_aeszkn_has_support()) {
        mbedtls_aeszkn_inverse_key((unsigned char *) RK,
            (const unsigned char *) (cty.buf + cty.rk_offset),
            ctx->nr);
        goto exit;
    }
...
static const unsigned char aes_test_ctr_key[][32] =
{
...
#error "Unrecognised value for MBEDTLS_AESNI_HAVE_CODE"
     ...
     */
    {
        ...
        for (i = 0; i < num_tests << 1; i++) {
            ...
            if (verbose != 0) {
                mbedtls_printf("  AES-CTR-%3u (%s): ", keybits,
                               (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
            }

            memcpy(nonce_counter, aes_test_ctr_nonce_counter[u], 16);
            memcpy(key, aes_test_ctr_key[u], keybits / 8);
